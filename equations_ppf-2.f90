! equations_ppf-KR-2-neutrino.f90
...
real(16) :: Omegam0,Omegar0,Omeganu0,lambdachit,phitinitial,chitinitial

real(dl) :: H0inMpcinsec

real(16) :: Hta2, Ha2

real(8) rhonu,rhonua4toMP2
integer nu_i
...
if (CP%Num_Nu_massive /= 0) then
    ...
    do nu_i = 1, CP%nu_mass_eigenstates
        call Nu_rho(1.0d0*nu_masses(nu_i),rhonu)
        rhonua4toMP2=rhonua4toMP2+rhonu*grhormass(nu_i)
    end do
end if
...
Omeganu0 = real(rhonua4toMP2/(3.0d0*H0inMpcinsec**2),16)

! write(*,*) "Omegam0 = ", Omegam0
! write(*,*) "Omegar0 = ", Omegar0
! write(*,*) "Omeganu0 = ", Omeganu0
...
Ha2 = H0inMpcinsec*Hta2(real(a,16), Omegam0, Omegar0, Omeganu0, lambdachit, phitinitial, chitinitial)
...
function Hta2(ainput,Omegam0input,Omegar0input,Omeganu0input, &
     & lambdachitinput,phitinitialinput,chitinitialinput)

implicit none

real(16) :: ainput,Omegam0input,Omegar0input,Omeganu0input, &
            &   lambdachitinput,phitinitialinput,chitinitialinput
real(16) :: Hta2

real(16) :: Omegam0,Omegar0,Omeganu0,lambdachit,phitinitial,chitinitial
common /inputparametersofHta2/ Omegam0,Omegar0,Omeganu0,lambdachit,phitinitial,chitinitial
...
real(16) :: Omegam0flag,Omegar0flag,Omeganu0flag,lambdachitflag,phitinitialflag, &
            & chitinitialflag
common /flags/ Omegam0flag,Omegar0flag,Omeganu0flag,lambdachitflag,phitinitialflag, &
            & chitinitialflag
...
rhonutinitial = rhonut(Neinitial)

! write(*,*) "rhomtinitial = ", rhomtinitial
! write(*,*) "rhortinitial = ", rhortinitial
! write(*,*) "rhonutinitial = ", rhonutinitial
...
!      &    12.0q0*Omegam0 - 12.0q0*Omeganu0 - 12.0q0*Omegar0)/phit0**4

lambdaphit = 12.0q0*(1.0q0 - Omegam0 - Omegar0 - Omeganu0)/phitinitial**4
...
phitpinitial = (-4.0q0*lambdaphit*phitinitial**3)/ &
&  ((4.0q0*rhomtinitial)/exp(3.0q0*Neinitial) + (4.0q0*rhortinitial)/exp(4.0q0*Neinitial) + &
&    lambdaphit*phitinitial**4 + lambdachit*chitinitial**4 + 4.0q0*rhonutinitial)

chitpinitial = (-4.0q0*lambdachit*chitinitial**3)/&
&  ((4.0q0*rhomtinitial)/exp(3.0q0*Neinitial) + (4.0q0*rhortinitial)/exp(4.0q0*Neinitial) + &
&    lambdaphit*phitinitial**4 + lambdachit*chitinitial**4 + 4.0q0*rhonutinitial)

Htinitial = sqrt(((4.0q0*rhomtinitial)/exp(3.0q0*Neinitial) + &
&      (4.0q0*rhortinitial)/exp(4.0q0*Neinitial) + lambdaphit*phitinitial**4 + &
&      lambdachit*chitinitial**4 + 4.0q0*rhonutinitial)/ &
&    (6.0q0 - phitpinitial**2 - chitpinitial**2))/sqrt(2.0q0)
...
function dHt(Ne,Ht,phit,chit,phitp,chitp)
    ...
    dHt = -((3.0q0*exp(Ne)*rhomtinitial + 4.0q0*rhortinitial)/exp(4.0q0*Ne) - &
    &     rhonutp(Ne) + &
    &     3.0q0*Ht**2*(phitp**2 + chitp**2))/&
    &  (6.0q0*Ht)
...
function dphitp(Ne,Ht,phit,chit,phitp,chitp)
    ...
    dphitp = -((lambdaphit*phit**3 + &
    &   (phitp*(-((3.0q0*exp(Ne)*rhomtinitial + 4.0q0*rhortinitial)/&
    &           exp(4.0q0*Ne)) + rhonutp(Ne) - &
    &        3.0q0*Ht**2*&
    &         (-6.0q0 + phitp**2 + chitp**2)))/6.0q0)/&
    & Ht**2)
...
function rhonut(Ne)
    ...
    real(16) :: a
    real(8) :: rhonua4toMP2, rhonu
    integer :: nu_i
    real(16) :: H0SI
    ! integer :: i, ni
    ! real(16) :: log10ainitial, log10afinal

    ! open(unit=11,file='rhonut.txt')

    aitoa0approx = 1.0q-5

    a = aitoa0approx*exp(Ne)

    ! write(*,*) "Ne = ", Ne
    ! write(*,*) "a = ", a

    H0SI = 1.0q3*real(CP%H0/Mpc, 16)
    ...
    ! a = 10.0q0**(log10ainitial + (log10afinal - log10ainitial)*real(i - 1,16)/real(ni - 1,16))
    ...
    if (CP%Num_Nu_massive /= 0) then
        !Get massive neutrino density relative to massless
        do nu_i = 1, CP%nu_mass_eigenstates
            call Nu_rho(real(a,8)*nu_masses(nu_i),rhonu)
            rhonua4toMP2=rhonua4toMP2+rhonu*grhormass(nu_i)
        end do
    end if
    ...
    ! rhonut = rhonua4toMP2/(H0SI**2*a**4)
    rhonut = rhonua4toMP2/(((CP%H0)*1.0d3/c)**2*a**4)

    ! write(*,*) "Ne = ", Ne
    ! write(*,*) "rhonut = ", rhonut

    ! write(11, "(2e25.16)") a, rhonut
...
function rhonutp(Ne)
    ...
    ! integer :: i, ni
    ! real(16) :: a, log10ainitial, log10afinal

    ! dNe = 1.0q-4

    ! open(unit=11,file='rhonutp.txt')
    ...
    ! rhonutp = (rhonut(Ne + dNe/2.0q0) - rhonut(Ne - dNe/2.0q0))/dNe

    rhonutp = (3.0q0*rhonut(-4.0q0*dNe + Ne) - 32.0q0*rhonut(-3.0q0*dNe + Ne) + &
    168.0q0*rhonut(-2.0q0*dNe + Ne) - 672.0q0*rhonut(-dNe + Ne) + &
    672.0q0*rhonut(dNe + Ne) - 168.0q0*rhonut(2.0q0*dNe + Ne) + &
    32.0q0*rhonut(3.0q0*dNe + Ne) - 3.0q0*rhonut(4.0q0*dNe + Ne))/ &
    (840.0q0*dNe)

    ! write(*,*) "Ne = ", Ne
    ! write(*,*) "rhonutp = ", rhonutp

    ! write(11, "(2e25.16)") a, rhonutp
    !
    ! end do
    !
    ! close(11)
