...
!quad precision splines from DG
    subroutine splineq(x,y,n,d11,d1n,d2)
    use Precision
    integer, intent(in) :: n
    real(16), intent(in) :: x(n), y(n), d11, d1n
    real(16), intent(out) :: d2(n)
    integer i
    real(16) xp,qn,sig,un,xxdiv,u(n-1),d1l,d1r
    ...
    do i=2,n-1
        ...
        sig=(x(i)-x(i-1))*xxdiv
        xp=1.q0/(sig*d2(i-1)+2.q0)

        d2(i)=(sig-1.q0)*xp

        u(i)=(6.q0*(d1r-d1l)*xxdiv-sig*u(i-1))*xp
    end do
    d1l=d1r
    ...
    if (d1n>.99q30) then
        ...
        qn=0.5q0
        un=(3.q0/(x(n)-x(n-1)))*(d1n-d1l)
    endif

    d2(n)=(un-qn*u(n-1))/(qn*d2(n-1)+1.q0)
    do i=n-1,1,-1
        d2(i)=d2(i)*d2(i+1)+u(i)
    end do
    end subroutine splineq

subroutine splineq_out(xarr,yarr,yarr_buff,n,x,y)
  use precision
  integer n,llo_out,lhi_out,midp
  real(16) xarr(n),yarr(n),yarr_buff(n)
  real(16) x,y,a0_out,b0_out,ho_out
  ...
  y=a0_out*yarr(llo_out)+b0_out*yarr(lhi_out)+((a0_out**3.0q0-a0_out)*&
       yarr_buff(llo_out)+(b0_out**3.0q0-b0_out)*&
       yarr_buff(lhi_out))*ho_out*ho_out /6.q0                    
end subroutine splineq_out
